...
Landmark roles help screen reader users jump to major sections of a page quickly.

## Example: Accessible Error Summary
...
Move focus to the summary after a failed submission so screen reader users immediately hear what went wrong.
...
<picture>
  ...
  <img src="image-small.jpg" alt="Example photo" />
</picture>
```

Using the `<picture>` element ensures the right image size downloads for each device.
